...
This is a retention/deletion behavior: after the window passes, dead session history is removed from gmux's local state.

## Decision

Prune local dead sessions whose parseable `exited_at` is older than 7 days.

The prune removes the session from the in-memory store and relies on existing cleanup paths to remove persisted metadata/scrollback and project membership references. Peer-owned sessions are skipped because their owning gmuxd remains responsible for lifecycle state.

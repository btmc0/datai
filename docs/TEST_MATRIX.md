...
| `docs/stories/webui-terminal-pasture-skin.md` | Runtime Web UI keeps existing flows while using the Terminal Pasture palette plus sharp CodeUI-inspired micro-polish and add-workspace suggestion focus retention | yes | yes | no | yes | implemented | `pnpm --filter @jump/web test`; `pnpm --filter @jump/web lint`; `pnpm --filter @jump/web build`; `TMPDIR=/tmp GOWORK=$PWD/go.work go test ./services/jumpd/cmd/jumpd`; local rebuilt `jumpd status` on `127.0.0.1:8790`; embedded CSS token/no-CRT/no-blur micro-polish check; `git diff --check` |

...
| --- | --- | --- | --- | --- | --- | --- | --- |
| `docs/stories/remote-access-modes.md` | Local baseline plus `tsnet` and `relay` remote-access modes, with configurable local TCP bind and release-packaged `jump-relayd` | yes | yes | no | yes | in_progress | `TMPDIR=/tmp GOWORK=$PWD/go.work go test ./services/jumpd/internal/config ./services/jumpd/cmd/jumpd ./services/jump-relayd/cmd/jump-relayd`; temp `listen = "0.0.0.0"` daemon smoke; `./scripts/build.sh`; `GOWORK=$PWD/go.work go run github.com/goreleaser/goreleaser/v2@latest check`; `GOWORK=$PWD/go.work go run github.com/goreleaser/goreleaser/v2@latest release --snapshot --clean` archive contains `jump`, `jumpd`, `jump-relayd`; `bash -n scripts/install.sh`; prior relay proof: `go test ./packages/relayproto`; `go test ./services/jumpd/internal/tsauth`; `go test ./services/jumpd/internal/relayclient` |
...
| `docs/stories/terminal-hot-path-performance.md` | Terminal output should keep healthy clients responsive during bursts by dropping slow WebSocket clients, batching browser writes, and exposing debug perf counters without changing normal behavior | yes | yes | no | no | implemented | `TMPDIR=/tmp GOWORK=$PWD/go.work go test ./cli/jump/internal/ptyserver ./services/jumpd/internal/wsproxy ./services/jumpd/cmd/jumpd`; `pnpm --filter @jump/web test -- terminal-io.test.ts --runInBand`; `pnpm --filter @jump/web lint`; `pnpm --filter @jump/web build` |
| `docs/stories/web-terminal-mobile-ime.md` | Mobile Web terminal input keeps IME pre-edit text, Android delete-plus-insert corrections, and terminal backspace state synchronized so Vietnamese Telex does not lose or duplicate characters | yes | no | no | yes | implemented | `pnpm --filter @jump/web test -- mobile-input.test.ts terminal-input.test.ts`; `pnpm --filter @jump/web lint`; `./scripts/build.sh`; local `./bin/jumpd status`; manual Android Chrome/Gboard Vietnamese Telex verification |
| `docs/stories/web-terminal-mode-replay.md` | Browser clients attaching to an already-running TUI should replay active mouse/input terminal modes so clicks and keys remain interactive without refresh | yes | yes | no | no | implemented | `TMPDIR=/tmp GOWORK=$PWD/go.work go test -count=1 ./cli/jump/internal/ptyserver` (3x); `TMPDIR=/tmp GOWORK=$PWD/go.work go test -count=1 ./cli/jump/internal/ptyserver ./services/jumpd/internal/wsproxy ./services/jumpd/cmd/jumpd` |

...
// cmdWait implements `jump --wait [--timeout N] <id>`.
//
// The wait itself happens server-side: jumpd already subscribes to
...
func cmdWait(ref string, timeoutSecs int) int {
	...
	if sess.Peer != "" {
		fmt.Fprintf(os.Stderr, "jump: --wait is only supported for local sessions (%s is on peer %q)\n",
			shortID(sess.ID), sess.Peer)
		return 1
	}

	endpoint := jumpdBaseURL() + "/v1/sessions/" + url.PathEscape(sess.ID) + "/wait"
	if timeoutSecs > 0 {
	...
	case http.StatusOK:
		...
		if err := json.NewDecoder(resp.Body).Decode(&env); err != nil {
			fmt.Fprintln(os.Stderr, "jump: decode wait response:", err)
			return 1
		}
		...
		case "idle":
			return waitExitIdle
		case "died":
			fmt.Fprintf(os.Stderr, "jump: session %s died before becoming idle\n", shortID(sess.ID))
			return waitExitDied
		default:
			fmt.Fprintf(os.Stderr, "jump: unexpected wait reason %q\n", env.Data.Reason)
			return 1
		}
	case http.StatusRequestTimeout:
		fmt.Fprintf(os.Stderr, "jump: --wait timed out after %ds\n", timeoutSecs)
	...
	case http.StatusUnprocessableEntity:
		...
		fmt.Fprintf(os.Stderr, "jump: --wait not supported for this session: %s\n",
			extractMessage(body))
		return 1
	case http.StatusNotFound:
		// Distinct from "no idle signal": means the session id is
		// unknown to jumpd entirely.
		fmt.Fprintf(os.Stderr, "jump: session %s not found\n", shortID(sess.ID))
		return 1
	default:
		body, _ := io.ReadAll(resp.Body)
		fmt.Fprintf(os.Stderr, "jump: wait failed: %s: %s\n", resp.Status, extractMessage(body))
		return 1
	}
}

// extractMessage pulls the .error.message field out of jumpd's

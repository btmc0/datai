...
// cmdAttach implements `jump --attach <id>`.
//
// Opens a WebSocket to jumpd's /ws/{id} handler via the local Unix
// socket. jumpd proxies to the session's own Unix socket for local
...
// helper that `jump <cmd>` uses, so attach feels the same as an
...
func cmdAttach(ref string) int {
	...
	// A dead session has no backend socket for jumpd to dial, so the WS
	...
	if !sess.Alive {
		fmt.Fprintf(os.Stderr, "jump: session %s is not running (open the UI to resume)\n", shortID(sess.ID))
		return 1
	}

	if !localterm.IsInteractive() {
		fmt.Fprintln(os.Stderr, "jump: --attach requires an interactive terminal")
		return 1
	}
	...
	// Dial jumpd's WS endpoint through its Unix socket. nhooyr's Dial
	// accepts any HTTPClient, so we reuse the unix-socket transport
	// that jumpdClient uses for plain HTTP calls.
	dialHTTP := jumpdClient()
	// A long-lived WS can exceed the default 5s timeout; clear it here.
	dialHTTP.Timeout = 0

	wsURL := "ws://jumpd/ws/" + sess.ID
	conn, _, err := websocket.Dial(ctx, wsURL, &websocket.DialOptions{
		HTTPClient: dialHTTP,
	})
	if err != nil {
		fmt.Fprintf(os.Stderr, "jump: connect %s: %v\n", shortID(sess.ID), err)
		return 1
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "jump: attach: %v\n", err)
		return 1
	}
	defer attach.Detach()
	attach.Start()
	...
	case <-readErr:
		...
		// or jumpd hung up. Either way, we're done.
	case <-attach.Done():
		// Local tty closed (stdin EOF). Detach cleanly; session keeps
		// running just like jump <cmd> with SIGHUP.
	case sig := <-sigCh:

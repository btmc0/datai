...
// openUI implements the bare `jump` invocation: ensure jumpd is up,
// learn its TCP listen address and auth token from /v1/health, and
// hand those to the local browser.
func openUI() {
	ensureJumpd()

	// Wait for jumpd to be reachable before opening browser.
	client := jumpdClient()
	...
	if !ready {
		log.Fatalf("jumpd is not running (check %s/jumpd.log for errors)", os.TempDir())
	}

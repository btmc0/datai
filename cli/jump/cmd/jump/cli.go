...
// mode is the top-level action jump is being asked to perform.
type mode int
...
// flags captures the parsed jump-level options. Anything that influences
...
func parseCLI(args []string) (mode, *flags, []string, error) {
	...
	// runner stop-at-first-positional rule that protects `jump <cmd>
	// --cmd-flag` from having jump eat --cmd-flag does nothing useful
	// here — it just turns `jump --wait <id> --timeout 60` into a
...
func parseInterspersedFlags(fs *flag.FlagSet, rest []string) []string {
	...
	for len(remaining) > 0 {
		...
		// the user's text as the jump flag. Short-circuit here so the
		// suffix flows through verbatim. Realistically this matters
		// only for `jump --send <id> -- <text>` where <text> may start
		// with dashes.
		if remaining[0] == "--" {
...
// printUsage writes the jump usage synopsis. Shown on --help, on parse
// errors (with an error message prefix), and nowhere else.
func printUsage(w io.Writer) {
	fmt.Fprint(w, `jump — wrap any command in a managed session

Usage:
  jump                              open the web UI
  jump [--no-attach] <cmd> [args]   run a command in a new session
  jump -- <cmd> [args]              use -- if <cmd> starts with a dash

Session management:
  jump --list                       list known sessions
  jump --attach <id>                reattach to an existing session
  jump --tail <N> <id>              print the last N lines of a session
  jump --kill <id>                  terminate a session
  jump --send <id> [text]           send text (or stdin) to a session and submit it
  jump --send --no-submit <id> ...  send without the trailing carriage return
  jump --wait <id>                  block until session is idle (agent finished its turn)
  jump --wait --timeout N <id>      ... or fail after N seconds

Flags before the command apply to jump itself. Once the first positional
argument is seen, everything after is the command to run, verbatim.

For daemon management see 'jumpd --help'.
`)
}

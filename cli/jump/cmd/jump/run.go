...
import (
	...
	"github.com/sting8k/jump/cli/jump/internal/binhash"
	"github.com/sting8k/jump/cli/jump/internal/localterm"
	"github.com/sting8k/jump/cli/jump/internal/naming"
	"github.com/sting8k/jump/cli/jump/internal/ptyserver"
	...
	"github.com/sting8k/jump/packages/workspace"
)

// handshakeTimeout bounds how long the parent end of spawnDetached
// waits for the child to finish registering with jumpd. The child's
// registerWithJumpd loops up to 5 times with 500ms backoff, so the
...
// the session visible in the jump UI.
func runSession(args []string, attach bool) {
	// Nested jump detection: if we're running interactively inside an
	// existing jump session, re-exec as a detached headless process instead
	// of doing PTY passthrough (which would nest PTY-within-PTY). The
	// detached process registers with jumpd and the session appears in the
	// jump UI. The original process returns immediately to the parent shell.
	if os.Getenv("JUMP") == "1" && localterm.IsInteractive() {
		spawnDetached(args, "started "+strings.Join(args, " ")+" in background (visible in jump)", false)
		return
	}

	// Explicit --no-attach: spawn detached, wait for the child to finish
	// registering with jumpd, then print the session id on stdout so the
	...
	// Honour JUMP_RESUME_ID when the daemon passed one in (resume,
	...
	//
	// JUMP_RESUME_ID is intentionally distinct from JUMP_SESSION_ID:
	// the runner sets the latter for its child process (consumed by
	// adapters / hooks), so a nested `jump foo` inside an
	// interactive jump session inherits JUMP_SESSION_ID from the
	// parent and would otherwise try to re-bind the parent's id.
	// The dedicated JUMP_RESUME_ID is only ever set by the daemon
	...
	sessionID := os.Getenv("JUMP_RESUME_ID")
	if sessionID == "" {
		sessionID = naming.SessionID()
	}
	socketDir := os.Getenv("JUMP_SOCKET_DIR")
	if socketDir == "" {
		socketDir = "/tmp/jump-sessions"
	}
	...
	// JUMP_RESUME_ID lands in a window where the targeted session
	...
	if errors.Is(err, ptyserver.ErrSocketInUse) {
		log.Printf("jump: requested session id %s is in use; falling back to a fresh id", sessionID)
		sessionID = naming.SessionID()
		sockPath = filepath.Join(socketDir, sessionID+".sock")
		listener, err = ptyserver.BindSocket(sockPath)
	}
	...
	env := []string{
		"JUMP=1",
		"JUMP_SOCKET=" + sockPath,
		"JUMP_SESSION_ID=" + sessionID,
		"JUMP_ADAPTER=" + a.Name(),
		"JUMP_RUNNER_VERSION=" + version,
	}
	...
	// $XDG_STATE_HOME/jump/sessions/<id>/scrollback, in the same
	// per-session directory jumpd's sessionmeta package writes
	// meta.json into.
	...
	// Auto-start jumpd if not running (one-shot, never retried), then
	...
	go func() {
		defer close(regDone)
		ok := registerWithJumpd(sessionID, sockPath)
	...
	if interactive {
		...
		//   (but we still catch them on jump in case raw mode is somehow bypassed)
	...
	//
	// Bounded by registerWithJumpd's retry budget (≤2.5s).
	<-regDone

	// Deregister from jumpd (best-effort)
	deregisterFromJumpd(sessionID)
...
// spawnDetached re-execs jump with the given args as a setsid'd
// background process, disconnected from the current terminal. Used
// for both --no-attach and nested-jump scenarios: the child registers
// with jumpd and appears in the UI.
//
...
// capture the id with id=$(jump --no-attach foo) and use it
// immediately, without polling.
//
// When waitForRegistration is false, the parent prints msg on stderr
// and returns immediately. This is the nested-jump path: an
// interactive user runs `jump foo` from a shell already inside a
// jump session and sees the message at their prompt; the session
...
func spawnDetached(args []string, msg string, waitForRegistration bool) {
	...
	// args is the command-remainder after jump flag parsing, so it does
	// not contain any jump flags. The detached child sees no jump flags,
	...
	if waitForRegistration {
		...
		// in the child; JUMP_HANDSHAKE_FD tells the child which fd to
		// write to.

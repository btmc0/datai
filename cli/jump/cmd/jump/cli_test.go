...
func TestParseCLI(t *testing.T) {
	tests := []struct {
		...
		// turn `jump --wait <id> --timeout 60` into a silent foot-trap.
...
// after the wrapped command go to the child, not to jump. This is
...
func TestRunModeKeepsPOSIXRunnerSemantics(t *testing.T) {
	// `jump pi --some-pi-flag` — --some-pi-flag must reach pi as part
	// of the command, not be parsed as an unknown jump flag.

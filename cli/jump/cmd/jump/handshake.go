...
//
// After dispatching the ack, handshakeAck unsets JUMP_HANDSHAKE_FD so
// the (now-closed) fd doesn't get re-interpreted by any nested fork
// the runner spawns later (the user's command, or a `jump` invoked
// from within it).
const handshakeFDEnv = "JUMP_HANDSHAKE_FD"

// handshakeAck signals registration outcome to the parent process via
// the file descriptor named in JUMP_HANDSHAKE_FD. No-op when the env
// var is unset (the common case: this jump wasn't spawned with a
// handshake parent).
//
...
func handshakeAck(sessionID string, registered bool) {
	...
	// `jump` would otherwise call handshakeAck against a fd that
...
// openHandshakeFD returns a *os.File wrapping JUMP_HANDSHAKE_FD, or
...
func openHandshakeFD() *os.File {
	...
	return os.NewFile(uintptr(fd), "jump-handshake")
}

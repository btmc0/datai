...
func TestNewState(t *testing.T) {
	s := New(Config{
		...
		Cwd:        "/tmp",
		Kind:       "generic",
		SocketPath: "/tmp/jump-sessions/sess-test.sock",
	})
...
func TestJSONIncludesComputedTitle(t *testing.T) {
	...
	if parsed["title"] != "~/dev/jump" {
		t.Fatalf("expected computed title '~/dev/jump', got %v", parsed["title"])
	}
	if parsed["shell_title"] != "~/dev/jump" {

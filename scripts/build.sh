...
echo "→ Building gmux-relayd…"
(cd "$ROOT/services/gmux-relayd" && go build -ldflags "$LDFLAGS_COMMON" -o "$BIN/gmux-relayd" ./cmd/gmux-relayd)

echo ""
ls -lh "$BIN/gmuxd" "$BIN/gmux" "$BIN/gmux-relayd"
echo "✓ Build complete"

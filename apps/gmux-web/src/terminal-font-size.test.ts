...
describe('terminal font size preference', () => {
  ...
  it('loads stored preference or clamped default', () => {
    ...
    expect(loadTerminalFontSize(13, memoryStorage({ 'gmux.terminal.fontSize': '17' }))).toBe(17)
    expect(loadTerminalFontSize(13, memoryStorage({ 'gmux.terminal.fontSize': 'bad' }))).toBe(TERMINAL_FONT_SIZE_MIN)
  })

  it('saves clamped preference', () => {
    const storage = memoryStorage()
    expect(saveTerminalFontSize(99, storage)).toBe(TERMINAL_FONT_SIZE_MAX)
    expect(storage.getItem('gmux.terminal.fontSize')).toBe(String(TERMINAL_FONT_SIZE_MAX))
  })

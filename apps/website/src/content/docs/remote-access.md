...
> This is for restrictive networks where the machine running gmux can make outbound HTTPS/WebSocket connections but cannot accept inbound connections. It is not needed for the normal Tailscale flow above.

Run a relay server on a VPS:

```bash
gmux-relayd -listen :8791 -token '<strong-random-token>'
```

Terminate TLS with Caddy/nginx and proxy your public HTTPS origin to this listener; configure gmuxd with the resulting `wss://.../_gmux/agent` URL.

Put the same token and the relay WebSocket URL in `~/.config/gmux/host.toml` on the machine running gmux:
...
Browsers connect to the relay's public HTTPS origin. gmux's normal auth token/cookie flow still applies; the relay token only authenticates the gmuxd agent to the relay.

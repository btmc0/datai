---
title: projects.json
description: Reference for ~/.local/state/jump/projects.json — project definitions and match rules.
tableOfContents:
  maxHeadingLevel: 3
---

`~/.local/state/jump/projects.json` (or `$XDG_STATE_HOME/jump/projects.json`)

Projects control which sessions appear in the sidebar and how they're grouped. jumpd reads and writes this file. You can also edit it directly; changes are picked up on the next daemon restart. The UI's **Manage projects** modal is the primary editing interface.
...
```json
{
  ...
  "items": [
    ...
    },
    {
      "slug": "jump",
      "match": [
        { "remote": "github.com/sting8k/jump" },
        { "path": "~/dev/jump" }
      ],
...
|-------|------|-------------|
| `version` | `number` | Schema version. Currently `2`. Managed by jumpd; do not change manually. |
...
| `sessions` | `string[]` | no | Session keys (resume keys or IDs) assigned to this project. Managed by jumpd; preserved but not required when editing. |
...
This matches sessions in `~/dev/jump`, `~/dev/jump/src`, `~/dev/jump/.grove/teak`, etc.
...
All of these session remotes would match: `https://github.com/sting8k/jump.git`, `git@github.com:sting8k/jump.git`, `ssh://git@github.com/sting8k/jump`.
...
This matches sessions started from `$HOME` itself, but not `~/dev/jump` or any other subdirectory. The default "home" project uses this to avoid catching every session.
...
1. **Path specificity**: the project with the longest matching path wins. A session in `~/dev/jump/.grove/teak/src` matches `~/dev/jump/.grove/teak` over `~/dev/jump`.
...
jumpd validates the file on load. Invalid state is rejected with an error. Rules:

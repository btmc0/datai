...
describe('resolveTarget', () => {
  test('uses selected session when it is in the list', () => {
    const sessions = [
      sess({ id: 'a', cwd: '/home/mg/dev/jump' }),
  ...
  test('ignores selectedId when it is not in the session list', () => {
    ...
    expect(target).toEqual({ peer: undefined, cwd: '/home/mg/dev/jump' })
  })
  ...
  test('falls back to fallbackCwd when no sessions', () => {
    const target = resolveTarget([], null, '/home/mg/dev/jump')
    expect(target).toEqual({ cwd: '/home/mg/dev/jump' })
  })
...
describe('formatTarget', () => {
  test('shows short cwd for local target', () => {
    expect(formatTarget({ cwd: '/home/mg/dev/jump' })).toBe('~/dev/jump')
  })

...
describe('matchSession', () => {
  ...
  it('matches by remote URL', () => {
    const sess = makeSession({
      id: 's2', cwd: '/other',
      remotes: { origin: 'git@github.com:sting8k/jump.git' },
    })
    expect(matchSession(sess, projects)?.slug).toBe('jump')
  })
  ...
  it('uses project paths when a session has no remotes', () => {
    const sess = makeSession({ id: 's4', cwd: '/dev/jump/src' })
    expect(matchSession(sess, projects)?.slug).toBe('jump')
  })
  ...
  it('respects host-scoped path rules', () => {
    const projects: ProjectItem[] = [
      { slug: 'local-app', match: [{ path: '/repo', hosts: [''] }] },
      { slug: 'laptop-app', match: [{ path: '/repo', hosts: ['laptop'] }] },
    ]

    expect(matchSession(makeSession({ id: 'local', cwd: '/repo/src' }), projects)?.slug).toBe('local-app')
    expect(matchSession(makeSession({ id: 'remote', cwd: '/repo/src', peer: 'laptop' }), projects)?.slug).toBe('laptop-app')
    expect(matchSession(makeSession({ id: 'other-host', cwd: '/repo/src', peer: 'server' }), projects)).toBeNull()
  })

  it('lets remote-backed child projects beat a vague parent path', () => {
    const projects: ProjectItem[] = [
      { slug: 'mg', match: [{ path: '/home/mg' }] },
      { slug: 'jump', match: [{ remote: 'github.com/sting8k/jump' }, { path: '/home/mg/dev/jump' }] },
      { slug: 'dots', match: [{ remote: 'github.com/mgabor3141/dots' }, { path: '/home/mg/.local/share/chezmoi' }] },
    ]

    const jumpSession = makeSession({
      id: 'g1',
      cwd: '/home/mg/dev/jump/src',
      remotes: { origin: 'git@github.com:sting8k/jump.git' },
    })
    expect(matchSession(jumpSession, projects)?.slug).toBe('jump')
  ...
  it('prefers most specific path over remote match', () => {
    const projects: ProjectItem[] = [
      { slug: 'teak', match: [{ path: '/home/user/dev/jump/.grove/teak' }] },
      { slug: 'jump', match: [{ remote: 'github.com/sting8k/jump' }, { path: '/home/user/dev/jump' }] },
    ]

    // Session in teak's directory: teak path is more specific than jump path.
    const sess = makeSession({
      id: 'w1',
      cwd: '/home/user/dev/jump/.grove/teak/src',
      remotes: { origin: 'git@github.com:sting8k/jump.git' },
    })
    ...
    expect(matchSession(sess2, projects)?.slug).toBe('jump')
  })
  ...
  it('exact path matches only the exact directory, not subdirs', () => {
    ...
    // Session under ~/dev/jump: matches jump, NOT home.
    expect(matchSession(makeSession({ id: 'g1', cwd: '~/dev/jump/src' }), projects)?.slug).toBe('jump')

...
function ruleMatchesHost(rule: { hosts?: string[] }, host: string): boolean {
  if (!rule.hosts?.length) return true
 ...
 * Host-scoped rules mirror the backend: empty host means local, session.peer
...
export interface HostNode {
   ...
   * array for the local jumpd. E.g. `['workstation']` for a direct peer,

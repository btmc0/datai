 ...
 *
 * Usage: open devtools console, run __jumpCopySession()
 *
...
function generateFile(session: Session, terminal: string, cursorX: number, cursorY: number): string {
  ...
  lines.push(`  socket_path: '/tmp/jump-sessions/${session.id}.sock',`)
...
export function installCopySession(): void {
  ;(window as any).__jumpCopySession = () => {
    const doCopy = () => {
      const session = (window as any).__jumpSession as Session | null
      const term = (window as any).__jumpTerm as Terminal | null

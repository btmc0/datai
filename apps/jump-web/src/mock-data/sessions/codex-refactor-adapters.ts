...
export default {
  ...
  unread: false,
  socket_path: '/tmp/jump-sessions/sess-4ab4995b.sock',
  peer: 'laptop',
  cursorX: 2,
  cursorY: 70,
  terminal: `
${DIM}╭───────────────────────────────────────╮${RST}
${DIM}│ directory: ${RST}~/dev/jump                 ${DIM}│${RST}
${DIM}╰───────────────────────────────────────╯${RST}
...
${DIM}• ${RST}I’ve mapped the main adapter surface area. Next I’m reading the interface definitions, registry, and one runtime path on each side
  (${CYAN}jump ${RST}and ${CYAN}jumpd${RST}) to separate structural issues from adapter-specific duplication.
...
${C1}                                                                                                                           ${RST}
  ${DIM}gpt-5.4 default · 67% left · ~/dev/jump${RST}`,

...
Playwright suite that drives a real `jumpd` + `jump` against a real
...
1. **Socket discovery.** Sockets live in `$JUMP_SOCKET_DIR` (default
   `/tmp/jump-sessions`). `jumpd start` re-execs to daemonize and
   strips every `JUMP_*` env var on the way, so a `start`-launched
   daemon falls back to the operator's `/tmp/jump-sessions`.
   **Fix:** spawn `jumpd run` directly from `global-setup.ts`. Keeps
...
for pi/claude/codex into `$JUMP_TEST_HOME` (set by global-setup). Two
specs use it:

- `tests/conversation-fixtures.spec.ts` is a drift-detector. It
  pre-seeds one fixture per adapter via global-setup *before* jumpd
...
- `__jumpTerm` — the live xterm instance, exposed for assertions on
  rows/cols/scroll state.
- `__jumpNavigateToSession(id)` — routes to a session by ID via the
...
- `apiGet<T>(urlPath)` — bearer-auth wrapper around `fetch` against
  `127.0.0.1:${JUMPD_TEST_PORT}`. Returns `{ status, body }`. Body is
...
- Use `__jumpTerm` for assertions about terminal state. Don't poke

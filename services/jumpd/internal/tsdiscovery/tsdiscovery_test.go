...
// jumpHealth returns a valid /v1/health response for the given hostname.
func jumpHealth(hostname string) http.HandlerFunc {
...
func TestProbe_JumpInstance(t *testing.T) {
	srv := httptest.NewTLSServer(jumpHealth("desktop"))
	defer srv.Close()
...
func TestProbe_NonJumpService(t *testing.T) {
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte("<html>not jump</html>"))
	}))
	defer srv.Close()
	...
	if ok {
		t.Fatal("expected probe to fail for non-jump service")
	}
}
...
func TestLooksLikeJump(t *testing.T) {
	w := New(Config{HostnamePrefix: "jump"})
	tests := []struct {
		hostname string
		want     bool
	}{
		...
		{"jump-dev", true},
		{"jump-dev-kauri", true},
		{"jumpfoo", false}, // no dash separator
		{"desktop", false},
		{"", false},
	}
	for _, tt := range tests {
		if got := w.looksLikeJump(tt.hostname); got != tt.want {
			t.Errorf("looksLikeJump(%q) = %v, want %v", tt.hostname, got, tt.want)
		}
	}
}
...
func TestStatePersistence(t *testing.T) {
	...
	if !state["desktop.ts.net"] {
		t.Error("expected desktop.ts.net to be jump")
	}
	if state["phone.ts.net"] {
		t.Error("expected phone.ts.net to NOT be jump")
	}
}
...
func TestStatePersistence_ReregistersOnLoad(t *testing.T) {
	...
	state := discoveryState{
		Devices: map[string]*deviceEntry{
			"node-1": {FQDN: "desktop.ts.net", PeerName: "desktop", IsJump: true, ProbedAt: time.Now().Format(time.RFC3339)},
		},
	}
...
func TestHandleNetMap_NewOnlinePeer(t *testing.T) {
	...
	if !state[fqdn] {
		t.Error("expected device to be cached as jump")
	}
}

func TestHandleNetMap_PeerComesOnline(t *testing.T) {
	srv := httptest.NewTLSServer(jumpHealth("laptop"))
	defer srv.Close()
...
func TestHandleNetMap_OfflinePeersList(t *testing.T) {
	...
	nm := makeNetMap(
		makeNode("node-1", "jump-dev.ts.net", "jump-dev", false),
		makeNode("node-2", "desktop.ts.net", "desktop", false),
		makeNode("node-3", "jump-hs.ts.net", "jump-hs", true), // online, won't be in offline list
	)
	...
	if offline[0].Name != "jump-dev" {
		t.Errorf("offline peer name = %q, want %q", offline[0].Name, "jump-dev")
	}
}

func TestHandleNetMap_ConfirmedJumpNotReprobed(t *testing.T) {
	probeCount := 0
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		probeCount++
		json.NewEncoder(w).Encode(map[string]any{
			"ok":   true,
			"data": map[string]any{"service": "jumpd", "hostname": "desktop"},
		})
	}))
	defer srv.Close()
	...
	// Second identical netmap: should NOT re-probe (confirmed jump).
	w.handleNetMap(context.Background(), nm)
	if probeCount != 1 {
		t.Errorf("expected no re-probe for confirmed jump, got %d probes", probeCount)
	}
}
...
func TestHandleNetMap_SelfExcluded(t *testing.T) {
	...
	w := New(Config{
		Manager:  mgr,
		StateDir: t.TempDir(),
		SelfFQDN: "jump.my.ts.net",
	})

	// Self node in the netmap (matching SelfFQDN).
	nm := makeNetMap(makeNode("self-node", "jump.my.ts.net", "jump", true))
	w.handleNetMap(context.Background(), nm)
...
func TestHandleNetMap_ConfirmedJumpReconnects(t *testing.T) {
	// A confirmed jump peer goes offline then comes back online.
	...
	if probeCount != 1 {
		t.Errorf("expected no re-probe for confirmed jump peer, got %d total probes", probeCount)
	}
}
...
func TestHandleNetMap_NonJumpCooldown(t *testing.T) {
	// A non-jump device should not be re-probed on every netmap update.
	probeCount := 0
	srv := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		probeCount++
		w.Write([]byte("not jump"))
	}))
	defer srv.Close()
	...
	// First netmap: probes, finds non-jump.
	w.handleNetMap(context.Background(), nm)
	if probeCount != 1 {
		t.Fatalf("expected 1 probe, got %d", probeCount)
	}

// Package tsdiscovery discovers jumpd instances on the local tailnet.
//
...
// determine whether they're running jumpd, and discovered instances are
// registered as peers via the peering Manager.
//
// Results are cached to disk so known jump devices are re-registered
...
type Config struct {
	...
	// suspected jump devices that are offline (never probed).
	// Defaults to "jump".
	HostnamePrefix string
}

// OfflinePeer describes a jump device visible on the tailnet but
...
// Watcher subscribes to tailnet changes and discovers jumpd instances.
type Watcher struct {
	cfg    Config
	...
	offline []OfflinePeer                         // suspected-offline jump peers
...
type peerSnapshot struct {
	fqdn     string
	hostname string // tailscale HostName (e.g. "jump-dev")
	online   bool
}
...
type deviceEntry struct {
	FQDN     string `json:"fqdn"`
	PeerName string `json:"peer_name,omitempty"` // set when IsJump is true
...
// OfflinePeers returns tailnet devices that look like jump instances
...
func (w *Watcher) Start() {
	// Re-register known jump peers immediately (without re-probing).
	...
	for _, d := range w.state.Devices {
		if d.IsJump && d.PeerName != "" && !w.isManualPeer(d.FQDN) {
...
func (w *Watcher) handleNetMap(ctx context.Context, nm *netmap.NetworkMap) {
	...
	for id, cs := range curr {
		...
		case existed && !ps.online && cs.online:
			// Came online. Probe if not already confirmed jump.
			if cached != nil && cached.IsJump {
		...
		case existed && cs.online && cached != nil && !cached.IsJump:
			// Still online, previously probed as non-jump: re-probe
			// in case jump was installed or updated since last check.
	...
	// offline and not already confirmed jump (those are managed peers).
...
func (w *Watcher) handleOnlinePeer(ctx context.Context, nodeID string, cs peerSnapshot) bool {
	...
	if w.isManualPeer(cs.fqdn) {
		...
		w.state.Devices[nodeID] = &deviceEntry{
			FQDN:     cs.fqdn,
			IsJump:   false, // don't auto-discover, user manages this
...
// looksLikeJump returns true if the hostname matches the configured
// prefix pattern (e.g. "jump-dev" matches prefix "jump").
func (w *Watcher) looksLikeJump(hostname string) bool {
	if hostname == "" {
		return false
	}
...
// probe checks whether the device at fqdn is running jumpd.
...
func (w *Watcher) probe(ctx context.Context, fqdn string) (hostname string, ok bool) {
	...
	if !envelope.OK || envelope.Data.Service != "jumpd" {
		return "", false // not jump, no need to log
	}
	if envelope.Data.Hostname == "" {
		log.Printf("tsdiscovery: probe %s: jumpd found but hostname field missing (upgrade jump on that machine)", fqdn)
		return "", false
	}
...
func (w *Watcher) findNameConflict(nodeID, peerName string) string {
	...
	for id, d := range w.state.Devices {
		if id != nodeID && d.IsJump && d.PeerName == peerName {
			return d.FQDN
		}
	}
	return ""
...
func (w *Watcher) isManualPeer(fqdn string) bool {
	for _, u := range w.cfg.ManualPeerURLs {
		// Manual URLs are like "https://jump.tailnet.ts.net" or
...
func (w *Watcher) SetCacheEntry(nodeID, fqdn, peerName string, isJump bool) {
	w.mu.Lock()
	defer w.mu.Unlock()
	w.state.Devices[nodeID] = &deviceEntry{
		FQDN:     fqdn,

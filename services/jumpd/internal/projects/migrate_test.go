...
func TestMigrateV1ToV2(t *testing.T) {
  ...
  "items": [
	...
	// Item 0: jump — remote + canonicalized path.
	jump := state.Items[0]
	if jump.Slug != "jump" {
		t.Errorf("item 0 slug = %q", jump.Slug)
	}
	if len(jump.Match) != 2 {
		t.Fatalf("jump: expected 2 match rules, got %d", len(jump.Match))
	}
	if jump.Match[0].Remote != "github.com/sting8k/jump" {
		t.Errorf("jump rule 0: remote = %q", jump.Match[0].Remote)
	}
	if jump.Match[1].Path != "~/dev/jump" {
		t.Errorf("jump rule 1: path = %q, want ~/dev/jump", jump.Match[1].Path)
	}
	if len(jump.Sessions) != 2 || jump.Sessions[0] != "hub-protocol" {
		t.Errorf("jump sessions = %v", jump.Sessions)
	}

	// Item 1: tmp — path outside $HOME stays canonical absolute.
	tmp := state.Items[1]
...
func TestMigrateV2Noop(t *testing.T) {
  ...
  "items": [
    {
      "slug": "jump",
      "match": [{"remote": "github.com/sting8k/jump"}, {"path": "~/dev/jump"}],
	...
	if state.Items[0].Match[0].Remote != "github.com/sting8k/jump" {
		t.Errorf("remote = %q", state.Items[0].Match[0].Remote)
	}
	if state.Items[0].Match[1].Path != "~/dev/jump" {

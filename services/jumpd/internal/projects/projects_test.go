...
func TestSaveAndLoad(t *testing.T) {
	...
	original := &State{
		...
		Items: []Item{
			{Slug: "jump", Match: []MatchRule{{Remote: "github.com/sting8k/jump"}, {Path: "/home/user/dev/jump"}}},
	...
	if loaded.Items[0].Slug != "jump" || loaded.Items[0].Match[0].Remote != "github.com/sting8k/jump" {
...
func TestSaveCreatesNestedDir(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "nested", "state", "jump")
...
func TestValidateNestedPathsAllowed(t *testing.T) {
	s := &State{Items: []Item{
		{Slug: "parent", Match: []MatchRule{{Path: "/home/user/dev/jump"}}},
		{Slug: "child", Match: []MatchRule{{Path: "/home/user/dev/jump/.grove/teak"}}},
	}}
	if err := s.Validate(); err != nil {
...
func TestMatchPathBased(t *testing.T) {
	s := &State{Items: []Item{
		{Slug: "jump", Match: []MatchRule{{Path: "/home/user/dev/jump"}}},
	}}

	// Exact match on cwd.
	if m := s.Match(MatchParams{Cwd: "/home/user/dev/jump", WorkspaceRoot: ""}); m == nil || m.Slug != "jump" {
		t.Error("expected match on exact cwd")
	}
	// Subdirectory match.
	if m := s.Match(MatchParams{Cwd: "/home/user/dev/jump/src", WorkspaceRoot: ""}); m == nil || m.Slug != "jump" {
		t.Error("expected match on subdirectory")
	}
	// Match via workspace_root.
	if m := s.Match(MatchParams{Cwd: "/somewhere/else", WorkspaceRoot: "/home/user/dev/jump"}); m == nil || m.Slug != "jump" {
	...
	if m := s.Match(MatchParams{Cwd: "/home/user/dev/jump-other", WorkspaceRoot: ""}); m != nil {
...
func TestMatchRemoteBased(t *testing.T) {
	...
	remotes := map[string]string{"origin": "https://github.com/sting8k/jump.git"}
	if m := s.Match(MatchParams{Cwd: "/any/dir", WorkspaceRoot: "", Remotes: remotes}); m == nil || m.Slug != "jump" {
		t.Error("expected match on HTTPS remote")
	}
	// Match via SSH-style remote.
	remotes = map[string]string{"origin": "git@github.com:sting8k/jump.git"}
	if m := s.Match(MatchParams{Cwd: "/any/dir", WorkspaceRoot: "", Remotes: remotes}); m == nil || m.Slug != "jump" {
		t.Error("expected match on SSH remote")
	}
	// Match on upstream, not just origin.
	remotes = map[string]string{
		"origin":   "git@github.com:fork/jump.git",
		"upstream": "https://github.com/sting8k/jump",
	}
	if m := s.Match(MatchParams{Cwd: "/any/dir", WorkspaceRoot: "", Remotes: remotes}); m == nil || m.Slug != "jump" {
...
func TestMatchPathSpecificityOverRemote(t *testing.T) {
	s := &State{Items: []Item{
		{Slug: "teak", Match: []MatchRule{{Path: "/home/user/dev/jump/.grove/teak"}}},
		{Slug: "jump", Match: []MatchRule{{Remote: "github.com/sting8k/jump"}, {Path: "/home/user/dev/jump"}}},
	}}

	remotes := map[string]string{"origin": "git@github.com:sting8k/jump.git"}

	// Session in teak directory with the jump remote: the more specific
	// path match (teak) wins over the remote match (jump).
	m := s.Match(MatchParams{Cwd: "/home/user/dev/jump/.grove/teak/src", WorkspaceRoot: "", Remotes: remotes})
	if m == nil || m.Slug != "teak" {
		t.Errorf("expected teak (most specific path), got %v", m)
	}
	// Session in main jump directory: jump path matches and remote also matches.
	m = s.Match(MatchParams{Cwd: "/home/user/dev/jump/src", WorkspaceRoot: "", Remotes: remotes})
	if m == nil || m.Slug != "jump" {
		t.Errorf("expected jump, got %v", m)
	}
	// Session with remote but no path match: remote fallback.
	m = s.Match(MatchParams{Cwd: "/other/dir", WorkspaceRoot: "", Remotes: remotes})
	if m == nil || m.Slug != "jump" {
		t.Errorf("expected jump (remote fallback), got %v", m)
	}
}
...
func TestMatchLongestPrefixWins(t *testing.T) {
	...
	m := s.Match(MatchParams{Cwd: "/home/user/dev/jump/.grove/teak/file.go", WorkspaceRoot: ""})
	if m == nil || m.Slug != "child" {
		t.Errorf("expected child (longest prefix), got %v", m)
	}
	// Session in jump root: parent wins.
	m = s.Match(MatchParams{Cwd: "/home/user/dev/jump/src/main.go", WorkspaceRoot: ""})
...
func TestMatchSpecificChildPathBeatsVagueParentEvenForRemoteProject(t *testing.T) {
	s := &State{Items: []Item{
		...
		{Slug: "jump", Match: []MatchRule{{Remote: "github.com/sting8k/jump"}, {Path: "/home/mg/dev/jump"}}},
		{Slug: "dots", Match: []MatchRule{{Remote: "github.com/mgabor3141/dots"}, {Path: "/home/mg/.local/share/chezmoi"}}},
	}}

	m := s.Match(MatchParams{Cwd: "/home/mg/dev/jump/src", WorkspaceRoot: "", Remotes: map[string]string{"origin": "git@github.com:sting8k/jump.git"}})
	if m == nil || m.Slug != "jump" {
		t.Errorf("expected jump, got %v", m)
	}
...
func TestMatchPathProjectStillTakesPrecedenceOverRemoteFallback(t *testing.T) {
	...
	// Path-matched project (teak) should win over remote project's path fallback (jump).
	m := s.Match(MatchParams{Cwd: "/home/user/dev/jump/.grove/teak/src", WorkspaceRoot: ""})
	if m == nil || m.Slug != "teak" {
...
func TestSlugFromRemote(t *testing.T) {
	tests := []struct {
		...
		{"https://github.com/sting8k/jump.git", "jump"},
...
func TestDiscoveredBasic(t *testing.T) {
	s := &State{Items: []Item{
		{Slug: "jump", Match: []MatchRule{{Remote: "github.com/sting8k/jump"}, {Path: "/dev/jump"}}},
	}}

	sessions := []SessionInfo{
		// Matches jump (should be excluded from discovered).
		{ID: "s1", Cwd: "/dev/jump", Remotes: map[string]string{"origin": "git@github.com:sting8k/jump.git"}},
...
func TestDiscoveredMergesWorkspaceRoot(t *testing.T) {
	s := &State{}
	sessions := []SessionInfo{
		{ID: "s1", Cwd: "/dev/jump", WorkspaceRoot: "/dev/jump"},
		{ID: "s2", Cwd: "/dev/jump/.grove/teak", WorkspaceRoot: "/dev/jump"},
	}
...
func TestDiscoveredAllMatched(t *testing.T) {
	s := &State{Items: []Item{
		{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}},
	}}
	sessions := []SessionInfo{
		{ID: "s1", Cwd: "/dev/jump/src"},
	}
...
func TestRemoveSession(t *testing.T) {
	s := State{Items: []Item{
		{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"a", "b", "c"}},
	}}
	if !s.RemoveSession("jump", "b") {
	...
	if s.RemoveSession("jump", "nonexistent") {
...
func TestRemoveSessionFromAll(t *testing.T) {
	s := State{Items: []Item{
		{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"a"}},
...
func TestManagerAutoAssign(t *testing.T) {
	...
	slug := mgr.AutoAssignSession(SessionInfo{
		ID: "sess-1", Cwd: "/dev/jump/src", WorkspaceRoot: "/dev/jump",
		Remotes: map[string]string{"origin": "github.com/sting8k/jump"},
		Alive:   true,
	})
	if slug != "jump" {
		t.Errorf("expected 'jump', got %q", slug)
	}
...
func TestManagerAutoAssignSlugUpgrade(t *testing.T) {
	...
	mgr.AutoAssignSession(SessionInfo{
		ID: "sess-1", Cwd: "/dev/jump", Alive: true,
	})
...
func TestManagerDismissSession(t *testing.T) {
	...
	mgr.Update(func(s *State) bool {
		s.Items = []Item{
			{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"key-1", "key-2"}},
		}
		return true
	})
...
func TestUnmatchedActiveCount(t *testing.T) {
	s := State{Items: []Item{
		{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"s1"}},
	}}
	sessions := []SessionInfo{
		{ID: "s1", Cwd: "/dev/jump", Alive: true},        // matched + in array
		{ID: "s2", Cwd: "/dev/jump", Alive: true},        // matched but not in array
	...
	// s3 and s5 are unmatched+alive. s2 matches jump (not counted). s4 is dead.
	if count != 2 {
...
func TestUnmatchedActiveCountAllMatched(t *testing.T) {
	...
	sessions := []SessionInfo{
		{ID: "s1", Cwd: "/dev/jump/src", Alive: true},
	}
...
func TestManagerDismissSessionByIDFallback(t *testing.T) {
	...
	mgr.Update(func(s *State) bool {
		s.Items = []Item{
			{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"sess-1"}},
		}
		return true
	})
...
func TestManagerCleanupSessions(t *testing.T) {
	...
	mgr.Update(func(s *State) bool {
		s.Items = []Item{
			{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"s1", "s2", "s3"}},
	...
	if got := state.Items[0].Sessions; len(got) != 2 || got[0] != "s1" || got[1] != "s3" {
		t.Errorf("jump sessions: expected [s1 s3], got %v", got)
	}
...
func TestManagerCleanupSessionsNoOrphans(t *testing.T) {
	...
	mgr.Update(func(s *State) bool {
		s.Items = []Item{
			{Slug: "jump", Match: []MatchRule{{Path: "/dev/jump"}}, Sessions: []string{"s1", "s2"}},
		}
		return true
	})
...
func TestManagerAutoAssignAllAlive(t *testing.T) {
	...
	sessions := []SessionInfo{
		...
		{ID: "s3", Cwd: "/dev/jump", Alive: false}, // dead: should be skipped
	...
	if len(state.Items[0].Sessions) != 1 || state.Items[0].Sessions[0] != "s1" {
		t.Errorf("jump sessions: expected [s1], got %v", state.Items[0].Sessions)
	}
...
func TestMatchHostScoping(t *testing.T) {
	...
	m = s.Match(MatchParams{Cwd: "/dev/jump/src", Host: "server"})
	if m == nil || m.Slug != "jump" {
		t.Errorf("server session in /dev/jump: expected 'jump', got %v", m)
	}
}
...
func TestNormalizePathExpandsTilde(t *testing.T) {
	...
	if got := NormalizePath("~/dev/jump"); got != home+"/dev/jump" {
		t.Errorf("NormalizePath(~/dev/jump) = %q, want %q", got, home+"/dev/jump")
	}
}
...
func TestMatchHomeProjectExact(t *testing.T) {
	...
	m = s.Match(MatchParams{Cwd: home + "/dev/jump/src"})
	if m != nil {

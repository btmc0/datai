...
func TestLoginPageHidesTokenFormByDefault(t *testing.T) {
	h := Middleware(testToken, okHandler())
	...
	if strings.Contains(rr.Body.String(), `name="token"`) {
		t.Error("default login page should not contain token input")
	}
	if !strings.Contains(rr.Body.String(), "token-bearing jump auth link") {
		t.Error("default login page should explain token link login")
	}
}

func TestLoginPageCanShowManualForm(t *testing.T) {
	h := Middleware(testToken, okHandler())
	req := httptest.NewRequest("GET", "/auth/login?form=1", nil)
	rr := httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	...
	if !strings.Contains(rr.Body.String(), `name="token"`) {
		t.Error("manual login page should contain token input")
	}
}
...
func TestLoginInvalidTokenRateLimited(t *testing.T) {
	h := Middleware(testToken, okHandler())

	for i := 0; i < authLimitMaxFailures; i++ {
		req := httptest.NewRequest("GET", "/auth/login?token=wrong", nil)
		req.RemoteAddr = "203.0.113.10:1234"
		rr := httptest.NewRecorder()
		h.ServeHTTP(rr, req)
		if rr.Code != http.StatusOK {
			t.Fatalf("attempt %d status = %d, want 200 before limit", i+1, rr.Code)
		}
	}
...
func TestLoginValidTokenStillWorksAfterFailures(t *testing.T) {
	h := Middleware(testToken, okHandler())

	for i := 0; i < authLimitMaxFailures+1; i++ {
		req := httptest.NewRequest("POST", "/auth/login", strings.NewReader(url.Values{"token": {"wrong"}}.Encode()))
		req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	...
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusSeeOther {
		t.Fatalf("valid token status = %d, want 303", rr.Code)
	}
}

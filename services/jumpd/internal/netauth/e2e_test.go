...
func TestE2E(t *testing.T) {
	// Build a realistic mux that mirrors jumpd's route structure.
	...
	t.Run("valid cookie grants access", func(t *testing.T) {
		req, _ := http.NewRequest("GET", srv.URL+"/v1/sessions", nil)
		req.AddCookie(&http.Cookie{Name: "jump-token", Value: testToken})
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("wrong cookie rejected", func(t *testing.T) {
		req, _ := http.NewRequest("GET", srv.URL+"/v1/sessions", nil)
		req.AddCookie(&http.Cookie{Name: "jump-token", Value: "wrong"})
		resp, err := http.DefaultClient.Do(req)
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("login page accessible without auth", func(t *testing.T) {
		...
		if strings.Contains(string(body), `action="/auth/login"`) {
			t.Error("default login page should hide form")
		}
	})
	...
	t.Run("full browser flow: redirect → token link → cookie → access", func(t *testing.T) {
		jar := &simpleCookieJar{}
		client := &http.Client{
			CheckRedirect: func(*http.Request, []*http.Request) error {
				return http.ErrUseLastResponse
		...
		// Step 3: GET /auth/login?token=<valid> → 303 + cookie
		resp, err = client.Get(srv.URL + "/auth/login?token=" + url.QueryEscape(testToken))
		if err != nil {
			t.Fatal(err)
		}
		resp.Body.Close()
...
// cookies with the exact name "jump-token", not similar names.
func TestE2ECookieNotSetOnWrongName(t *testing.T) {
	h := Middleware(testToken, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Error("should not reach handler")
	}))

	for _, name := range []string{"jump_token", "jump-Token", "token", "session"} {
		req := httptest.NewRequest("GET", "/v1/sessions", nil)

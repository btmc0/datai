...
func jumpContainer(id, name, ip, folder string) container {
	...
	return container{
		ID:     id,
		Name:   name,
		Env:    []string{"PATH=/usr/bin", "JUMPD_LISTEN=0.0.0.0"},
		Labels: labels,
		IP:     ip,
	}
}
...
func TestScan_DiscoversNewContainer(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("abc123full", "my-devcontainer", "172.17.0.2", "/home/user/dev/myproject"),
		},
...
func TestScan_RemovesStalePeer(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("abc123", "dev", "172.17.0.2", "/home/user/dev/myproject"),
		},
...
func TestScan_SkipsContainerWithoutJumpdEnv(t *testing.T) {
	mgr := setupManager(t)
	fd := &fakeDocker{
		containers: []container{{
			ID:   "plain-container",
	...
	if w.Tracked() != 0 {
		t.Fatalf("tracked = %d, want 0 (non-jump container)", w.Tracked())
	}
}
...
func TestScan_SkipsContainerWithoutToken(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("starting-up", "dev", "172.17.0.2", "/home/user/dev/proj"),
		},
...
func TestScan_RetriesReadTokenUntilReady(t *testing.T) {
	...
	fd := &callbackDocker{
		containers: []container{
			jumpContainer("slow", "dev", "172.17.0.2", "/home/user/dev/proj"),
		},
...
func TestScan_ParallelReadTokens(t *testing.T) {
	...
	fd := &callbackDocker{
		containers: []container{
			jumpContainer("fast", "fast-dev", "172.17.0.2", "/a"),
			jumpContainer("broken", "broken-dev", "172.17.0.3", "/b"),
			jumpContainer("slow", "slow-dev", "172.17.0.4", "/c"),
		},
		onRead: func(_ context.Context, id string) (string, error) {
			switch id {
			case "fast":
...
func TestScan_RetryReadTokenRespectsContextCancel(t *testing.T) {
	...
	fd := &callbackDocker{
		containers: []container{
			jumpContainer("stuck", "dev", "172.17.0.2", "/home/user/dev/proj"),
		},
...
func TestScan_SkipsContainerWithoutIP(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{{
			ID:   "no-network",
			Name: "dev",
			Env:  []string{"JUMPD_LISTEN=0.0.0.0"},
			IP:   "", // no IP
		}},
...
func TestScan_IdempotentForTrackedContainers(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("abc", "dev", "172.17.0.2", "/home/user/proj"),
		},
...
func TestScan_UniqueNamesForDuplicateProjects(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("aaa", "dev1", "172.17.0.2", "/home/user/work/myapp"),
			jumpContainer("bbb", "dev2", "172.17.0.3", "/home/other/myapp"),
		},
...
func TestScan_DoesNotConflictWithManualPeer(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("ccc", "dev", "172.17.0.2", "/home/user/server"),
		},
...
func TestScan_ContainerRestart(t *testing.T) {
	...
	fd := &fakeDocker{
		containers: []container{
			jumpContainer("old-id", "dev", "172.17.0.2", "/home/user/proj"),
		},
	...
	fd.containers = []container{
		jumpContainer("new-id", "dev", "172.17.0.3", "/home/user/proj"),
	}
...
func TestEvents_TriggerScan(t *testing.T) {
	...
	fd.setContainers([]container{
		jumpContainer("abc", "dev", "172.17.0.2", "/home/u/proj"),
	})
...
func TestEvents_RescanOnReconnect(t *testing.T) {
	...
	fd.mu.Lock()
	fd.containers = []container{jumpContainer("abc", "dev", "172.17.0.2", "/home/u/proj")}
	fd.mu.Unlock()
...
func TestWaitForSocket_AppearsLater(t *testing.T) {
	...
	mgr := setupManager(t)
	fd := &fakeDocker{
		containers: []container{jumpContainer("abc", "dev", "172.17.0.2", "/home/u/proj")},
		tokens:     map[string]string{"abc": "tok"},
		eventCh:    make(chan struct{}),
	}
	w := newWatcher(mgr, fd)
...
func TestIsJumpContainer(t *testing.T) {
	tests := []struct {
		name string
		env  []string
		want bool
	}{
		{"with JUMPD_LISTEN", []string{"PATH=/bin", "JUMPD_LISTEN=0.0.0.0"}, true},
		{"without JUMPD_LISTEN", []string{"PATH=/bin", "REDIS_PORT=6379"}, false},
		{"empty env", nil, false},
		{"partial match", []string{"JUMPD_LISTEN_OTHER=x"}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := container{Env: tt.env}
			if got := isJumpContainer(c); got != tt.want {
				t.Errorf("isJumpContainer = %v, want %v", got, tt.want)
			}
		})
	}
}

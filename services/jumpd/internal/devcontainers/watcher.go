// Package devcontainers discovers jumpd instances running in dev containers
// on the local Docker daemon and registers them as peers.
//
// Detection: containers with JUMPD_LISTEN in their environment (set by the
// jump devcontainer feature) are candidates. The watcher reads each
...
const (
	...
	// container whose jumpd is still starting up (auth-token file not
...
// running jumpd, and manages their lifecycle as peers in the peering
// manager.
type Watcher struct {
	mgr             *peering.Manager
	docker          dockerRunner
...
func (w *Watcher) scan(ctx context.Context) {
	...
	// Index jump containers by ID.
	jump := make(map[string]container)
	for _, c := range containers {
		if isJumpContainer(c) {
			jump[c.ID] = c
		}
	}
	...
	// Read tokens concurrently with bounded retries (jumpd inside the
	...
	for r := range results {
		...
		url := fmt.Sprintf("http://%s:%d", r.c.IP, jumpdPort)
...
// with tokenRetryDelay between attempts, so we catch jumpd instances
...
// isJumpContainer returns true if a container is running jumpd via the
// devcontainer feature. Detection: JUMPD_LISTEN env var is set by the
// feature's containerEnv.
func isJumpContainer(c container) bool {
	for _, e := range c.Env {
		if strings.HasPrefix(e, "JUMPD_LISTEN=") {
			return true
		}
	}
	return false

...
// the local filesystem so a TUI running in a jump session can read them
// by path. Image paste in jump-web POSTs bytes to the jumpd that owns
// the session; that jumpd uses a Writer to land the bytes in its own
...
// peer-forwarding HTTP layer: the request is forwarded to the jumpd
// running inside the container or on the peer machine, and that jumpd

...
func TestParseProcesses(t *testing.T) {
  ...
  10   1 12000 /Users/me/.local/bin/jump pi
  11  10 20000 pi
  12  11  3000 helper process
`)
	if got := procs[10].command; got != "/Users/me/.local/bin/jump pi" {
...
func TestSumTreeRSSIncludesRunnerAndChildTree(t *testing.T) {
	procs := map[int]processInfo{
		10: {pid: 10, ppid: 1, rssKB: 12000, command: "/Users/me/.local/bin/jump pi"},
	...
	child := procs[11]
	root := child.pid
	if parent, ok := procs[child.ppid]; ok && isJumpRunner(parent.command) {
		root = parent.pid
	}

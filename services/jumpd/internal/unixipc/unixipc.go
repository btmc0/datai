// Package unixipc manages the jumpd Unix socket for local IPC.
//
// The socket is the primary communication channel between the jump CLI
...
// Client returns an http.Client that connects to a jumpd Unix socket.
...
// Healthy checks if a jumpd is reachable and healthy at the given socket.
...
// Shutdown asks a running jumpd to shut down via its Unix socket,

...
//
// When JUMPD_DEV_PROXY is set (e.g. "http://localhost:5173"), all frontend
// requests are reverse-proxied to that URL instead. This lets the vite dev
// server handle HMR while jumpd handles API, WebSocket, and Tailscale auth.
func spaHandler() http.Handler {
	if devProxy := os.Getenv("JUMPD_DEV_PROXY"); devProxy != "" {
...
func devProxyHandler(target string) http.Handler {
	...
	if err != nil {
		log.Fatalf("JUMPD_DEV_PROXY: invalid URL %q: %v", target, err)
	}

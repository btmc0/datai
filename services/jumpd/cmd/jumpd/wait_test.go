...
// `jump --wait` is a no-op when the agent has already finished its
// turn before the wait call lands. This is the common case for
// composition (`jump --send X && jump --wait X` after the agent
...
// don't emit Working transitions, so `jump --wait` against them
// would return immediately with reason=idle and silently do the
// wrong thing for `jump make build && jump --wait <id>`-style
...
// this guard, `jump pi <prompt> --no-attach && jump --wait $id` would

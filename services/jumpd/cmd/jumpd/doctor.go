...
func runDoctor(stdout, stderr io.Writer) int {
	...
	_, _ = fmt.Fprintln(stdout, "jumpd doctor")

	cfg, cfgErr := config.Load()
	if cfgErr != nil {
		failures++
		_, _ = fmt.Fprintf(stdout, "✗ config invalid: %v\n", cfgErr)
		_, _ = fmt.Fprintf(stdout, "  Fix: edit %s and rerun `jumpd doctor`.\n", config.Path())
	} else {
	...
	if err != nil {
		...
		_, _ = fmt.Fprintln(stdout, "  Fix: run `jumpd start`.")
		return doctorExitCode(failures)
	}

	_, _ = fmt.Fprintf(stdout, "✓ daemon running: jumpd %s (%s)\n", health.Version, health.Status)
...
func doctorTsnet(stdout io.Writer, health daemonHealthData) int {
	if health.Tailscale == nil {
		...
		_, _ = fmt.Fprintln(stdout, "  Fix: restart the daemon with `jumpd restart`.")
		return 1
	}
	...
	if ts.AuthURL != "" {
		...
		_, _ = fmt.Fprintf(stdout, "  Fix: open %s, then rerun `jumpd doctor`.\n", ts.AuthURL)
		return 1
	}
	if !ts.Connected {
		_, _ = fmt.Fprintln(stdout, "✗ tsnet is still connecting")
		_, _ = fmt.Fprintln(stdout, "  Fix: wait a moment or check daemon logs with `jumpd log-path`.")
		return 1
	}
...
func doctorRelay(stdout io.Writer, cfg config.Config) int {
	...
	if err != nil {
		...
		_, _ = fmt.Fprintln(stdout, "  Fix: check relay.url, networking, and that jump-relayd is running.")
		return 1
	}
	defer resp.Body.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&health); err != nil || !health.OK {
		...
		_, _ = fmt.Fprintln(stdout, "  Fix: confirm relay.url points at jump-relayd.")
		return 1
	}
	if !health.AgentConnected {
		_, _ = fmt.Fprintln(stdout, "✗ relay server reachable but jumpd agent is not connected")

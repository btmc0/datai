...
func TestEnableTailscaleConfig_ExistingFileNoSection(t *testing.T) {
	...
	if strings.HasPrefix(content, "# jump") {
...
func TestEnableRelayConfig_ProducesValidConfig(t *testing.T) {
	cases := []struct {
		...
		{"existing relay values", "[remote]\nmode = \"relay\"\n\n[relay]\nurl = \"wss://old.example.com/_jump/agent\"\ntoken = \"old\"\n"},
	}
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			dir := t.TempDir()
			...
			if err := enableRelayConfig(cfgPath, " wss://relay.example.com/_jump/agent ", " secret ", " https://jump.example.com "); err != nil {
				t.Fatal(err)
			}
			...
			if cfg.Relay.URL != "wss://relay.example.com/_jump/agent" {
...
func TestEnableRelayConfig_RejectsTsnetMode(t *testing.T) {
	...
	err := enableRelayConfig(cfgPath, "wss://relay.example.com/_jump/agent", "secret", "")
	if err == nil {
...
func TestEnableRelayConfig_RejectsInvalidInput(t *testing.T) {
	...
	err := enableRelayConfig(cfgPath, "https://relay.example.com/_jump/agent", "secret", "")
	if err == nil {
...
func TestRelaySetup_UserDeclinesNoConfigChange(t *testing.T) {
	...
	cfgPath := filepath.Join(dir, "jump", "host.toml")
...
func TestDisplayStatus_NeedsLogin(t *testing.T) {
	...
	if !strings.Contains(out, "jumpd tsnet") {
		t.Errorf("should tell user to run jumpd tsnet again:\n%s", out)
	}
...
func TestDisplayStatus_Connected(t *testing.T) {
	...
	out := stdout.String()
	if !strings.Contains(out, "https://jump.tailnet.ts.net") {
...
`), 0o644); err != nil {
	...
	if !strings.Contains(out, "https://jump.example.com") {
		t.Errorf("missing public URL:\n%s", out)
	}
	if !strings.Contains(out, "wss://relay.example.com/_jump/agent") {

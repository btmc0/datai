...
func main() {
	...
	token := flag.String("token", "", "bearer token required for gmuxd agent")
	tokenFile := flag.String("token-file", "", "file containing bearer token required for gmuxd agent")
	agentPath := flag.String("agent-path", "/_gmux/agent", "WebSocket path for the gmuxd agent")
	flag.Parse()
	...
	log.Printf("gmux-relayd listening on %s", *listen)
...
func (r *relay) handleBrowser(w http.ResponseWriter, req *http.Request) {
	if req.URL.Path == "/_gmux/health" {
		r.handleHealth(w, req)
		return
	}
	agent := r.currentAgent()
	if agent == nil {
		writeError(w, http.StatusServiceUnavailable, "gmux agent not connected")
		return
	}

...
// unauthenticated request → redirect → token-link login → cookie → access.
func TestIntegrationFullLoginFlow(t *testing.T) {
	...
	if strings.Contains(string(body), `name="token"`) {
		t.Fatal("step 2: default login page should hide token input")
	}

	// Step 3: GET /auth/login?token=<valid> → redirect + cookie.

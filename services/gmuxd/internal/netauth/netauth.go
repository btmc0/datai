...
<title>gmux - Too Many Attempts</title></head>
...
<p style="font-size:.9em;line-height:1.5;margin:0;color:#999">Wait a few minutes, then open a fresh gmux auth link again.</p>
</div></body></html>`))
}
...
func serveLoginPage(w http.ResponseWriter, errMsg string, showForm bool) {
	...
	introHTML := `<p>This gmux instance requires authentication. Open a token-bearing gmux auth link to continue.</p>`
	formHTML := ``
	if showForm {
		introHTML = `<p>This gmux instance requires authentication. Enter the access token to continue.</p>`
...
<div class="card">
  ...
  ` + errorHTML + `
  ` + formHTML + `
  <p class="hint">Find your login link by running <code>gmuxd auth</code> on the host machine.</p>
</div>
</body>
</html>`))
}

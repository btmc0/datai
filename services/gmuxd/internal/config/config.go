...
type RelayConfig struct {
	// Enabled connects gmuxd to a gmux-relayd server over outbound WebSocket.
	Enabled bool `toml:"enabled"`

	// URL is the gmux-relayd agent WebSocket URL, e.g. wss://gmux.example.com/_gmux/agent.
	URL string `toml:"url"`

	// Token is the bearer token used to authenticate this gmuxd to gmux-relayd.
	Token string `toml:"token"`
}
